...
WALKING_UPDATES_PER_FRAME = 3
JUMPING_UPDATES_PER_FRAME = 3
...
class PlayerCharacter(arcade.Sprite):
    def __init__(self):
        ...
        texture = load_texture_pair_with_frame(f"{main_path}.png", 142, 53, 16, 22)
        self.jump_textures.append(texture)

        texture = load_texture_pair_with_frame(f"{main_path}.png", 166, 55, 22, 16)
        self.jump_textures.append(texture)

        texture = load_texture_pair_with_frame(f"{main_path}.png", 194, 53, 16, 22)
        self.jump_textures.append(texture)

        texture = load_texture_pair_with_frame(f"{main_path}.png", 238, 57, 22, 16)
        self.jump_textures.append(texture)
    ...
    def update_animation(self, delta_time: float = 1/60):
        ...
        if self.change_y != 0:
            self.cur_texture += 1
            if self.cur_texture > 2 * JUMPING_UPDATES_PER_FRAME:
                self.cur_texture = 0
            self.texture = self.jump_textures[self.cur_texture // JUMPING_UPDATES_PER_FRAME][self.character_face_direction]
        else:
            # Walking animation
            self.cur_texture += 1
            if self.cur_texture > 2 * WALKING_UPDATES_PER_FRAME:
                self.cur_texture = 0
            self.texture = self.walk_textures[self.cur_texture // WALKING_UPDATES_PER_FRAME][self.character_face_direction]
